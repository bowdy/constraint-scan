...
- **Delta** (default when a previous report exists): read only the last 24 hours, carry everything else forward from the previous report's state block, and send a short report. One sub-agent.
- **Full**: run the whole pipeline below and send the report. When it runs is set by the config's "Re-baseline" line: on a weekday such as Monday, or `never` (budget mode), in which case it runs only when the user asks for "full mode". It also runs when there is no previous report within 14 days, unless the config says `never`, in which case the delta agent bootstraps the state itself with wider windows and a higher tool-call cap.

Budget mode (config "Re-baseline: never", "Escalation: none", "Sender: delta agent") is the cheapest shape: exactly one Sonnet sub-agent per day with a cap of about 12 tool calls does the scan, the judgement, the writing and the send, and the orchestrator makes only the guard calls. That runs for well under $1 a day at API rates. The trade is that the world model is never re-baselined unless the user asks, so a moved constraint is adopted on one day's evidence with confidence capped at 0.5 and the report says so.
